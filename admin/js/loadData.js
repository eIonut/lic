    ...
    if (ajax_request.readyState == 4 && ajax_request.status == 200) {
      ...
      if (response.length > 0) {
        for (var count = 0; count < response.length; count++) {
          ...
          html += '<div class="card">';
          html +=
            '<img class="card-img-top" src="./upload/' +
            response[count].course_image +
            '">';
          html += '<div class="card-body">';
          html +=
            '<h5 class="card-title">' + response[count].course_name + " </h5>";
          html +=
            '<p class="card-text">' +
            response[count].course_description +
            "</p>";
          html +=
            '<a class="btn-hover color-2" href="delete_course_admin.php?course_name=' +
