...
function load_data(query = "") {
  ...
  ajax_request.open("POST", "processDataUser.php");
    ...
    if (ajax_request.readyState == 4 && ajax_request.status == 200) {
      ...
      if (response.length > 0) {
        for (var count = 0; count < response.length; count++) {
          ...
          html +=
            '<img class="card-img-top img-fluid" style="height: 150px; max-height: 75%;" src="admin/upload/' +
          ...
          html +=
            '<a class="btn d-block btn-primary mt-auto text-start pl-3 py-2 border-0 rounded-0 rounded-bottom w-100 d-flex justify-content-between align-items-center" style="background: linear-gradient(84.57deg, #1b3d7d 0%, #4a6db0 100%);" href="coursePage.php?id=' +
            response[count].id +
